angular.module( 'riass.broadcasting' )
	.controller( 'WidgetAnswerCtrl', [ '$scope', '$element', 'widgetService', function ( $scope, $element, widgetService ){
...
$scope.showQuestions = function () {
			for ( var i = 0; i < 5; i ++ ) {
				if ( $scope.widget.fields.table[i].column2_text ) return false;
			}
			return true;
		}
		var maxVotes = function () {
				var currentMax = 0;
				for ( var i = 0; i < 5; i ++ ) {
					if ( $scope.widget.fields.table[i].column2_text ) {
...
*/
	...
	.controller( 'WidgetAnswerCtrlEdit', [ '$scope', '$element', 'widgetService', 'utilsService', function ( $scope, $element, widgetService, utilsService ){
